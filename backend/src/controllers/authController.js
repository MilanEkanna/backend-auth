...
async function login(req, res, next) {
  try {
    ...
    const refresh = tokenService.generateRefreshToken(safeUser);
...
async function refresh(req, res, next) {
  try {
    ...
    let payload;
    try {
      payload = require('jsonwebtoken').decode(refreshToken);
    } catch (err) {
      payload = null;
    }
    ...
    if (payload && payload.sub) {
      const freshUser = User.findById(payload.sub);
      if (freshUser) {
    ...
    const rotated = tokenService.rotateRefreshToken(refreshToken, safeUser);
